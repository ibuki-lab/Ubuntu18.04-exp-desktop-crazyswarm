...
class Vel_controller(object):
    ...
    # X_dotの計算
        ...
        self.X_err_sum += self.X_err*dt               # 積分項
        # print(self.Xd*(self.X_err - self.X_err_prev))

        # 学習データ取り
        self.X_dot = self.Xp*self.X_err + self.Xi*self.X_err_sum + self.Xd*(0.1*(self.X_err - self.X_err_prev) / dt + 0.9 * self.pre_X_dot)
        self.pre_X_dot = self.X_dot

    # Y_dotの計算
        self.Y_err_prev = self.Y_err
        self.Y_err = pos_des[1] - pos_now[1]
        self.Y_err_sum += self.Y_err*dt
