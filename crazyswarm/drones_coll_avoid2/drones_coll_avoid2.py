...
# 高度の合意制御
class drones_coll_avoid2(Frames_setup, Vel_controller):

    # このクラスの初期設定を行う関数
    def __init__(self, experiment_time, alpha, hight):
        # frames_setup, vel_controller の初期化
        super(Frames_setup, self).__init__(alpha, hight)
...
#######################################################################################
    ...
    　　　　      [ cf2 cf1 ]  [  cf5  ]  [    G    ]
                  ...
                  [    G    ]  [   H   ]  [ cf3 cf4 ]
                              実験パソコン
        """
        self.way_points = np.array([[[1.2, -1.2], [0, -1.2], [-1.2, -1.2]],
                                     [[1.2,    0], [0,    0], [-1.2,    0]],
                                     [[1.2,  1.2], [0,  1.2], [-1.2,  1.2]]])
        self.D1_ACE = np.array([self.way_points[0, 0], self.way_points[0, 2], self.way_points[1, 1]])
        self.D2_AEG = np.array([self.way_points[0, 0], self.way_points[1, 1], self.way_points[2, 0]])
        self.D3_IGE = np.array([self.way_points[2, 2], self.way_points[2, 0], self.way_points[1, 1]])
        self.D4_IEC = np.array([self.way_points[2, 2], self.way_points[1, 1], self.way_points[0, 2]])
        self.D5_BDHF = np.array([self.way_points[0, 1], self.way_points[1, 0], self.way_points[2, 1], self.way_points[1, 2]])
        self.Drones_waypoints = [self.D1_ACE, self.D2_AEG, self.D3_IGE, self.D4_IEC, self.D5_BDHF]
        ...
        while True:
            ...
            for cf, child_frame in zip(self.allcfs.crazyflies, self.child_frames):
                ...
                self.cmd_controller_output(cfs_pos_now=self.cfs_state,
                                     pos_des = self.Drones_waypoints[int(child_frame[2]) - 1],
                                     yaw_des=0.0,
                                     cur_cf_idx=i
                                     )
                # crazyflieに速度指令を送る
...
if __name__ == '__main__':
    ...
    drones_coll_avoid2(experiment_time, 20, 0.5).main()