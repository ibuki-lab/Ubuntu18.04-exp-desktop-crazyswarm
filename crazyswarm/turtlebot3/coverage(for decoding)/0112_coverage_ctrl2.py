...
#####################################################################################
    ...
    # 各crazyflieに指令を送る!子フレーム(cf?)と命令対象のcrazyflieが一致している事が絶対条件 !
        ...
        self.pnts_inits = self.pnts_init[1:,:]
    ...
    def main(self):
        ...
        while True:
            ...
            for cf, child_frame in zip(self.allcfs.crazyflies, self.child_frames):
                ...
                try:
                    t = self.tfBuffer.lookup_transform(self.world_frame, child_frame, rospy.Time(0))
                    self.tmp_pints = np.vstack((self.tmp_pints, np.array([t.transform.translation.x, t.transform.translation.y])))
...
######## ボロノイ分割をして重心（目標値を求める） ##########################################################################################
            ...
            # ボロノイ図を計算
                ...
                # # # 動いた後の位置を更新
                # self.pnts = self.get_newpnts(self.pnts, x_move, y_move)
...
###############################################################################################################
            ...
            # 目標値生成
                ...
                for i in range(len(self.pnts)):
                    pos_desired = [np.array(centroid[i])[0] ,np.array(centroid[i])[1] , hight_start]
