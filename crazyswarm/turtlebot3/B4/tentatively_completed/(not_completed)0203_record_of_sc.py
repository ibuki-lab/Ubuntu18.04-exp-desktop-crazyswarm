...
#################### 有界なボロノイ図を計算・描写するための関数 #########################
    ...
    def get_voronoi(self, bnd, pnts):
        ...
        # for i in range(len(gn_pnts)):
            ...
            vor_poly = [self.vor.vertices[v] for v in self.vor.regions[self.vor.point_region[i]]]
            ...
            i_cell = self.bnd_poly.intersection(Polygon(vor_poly))
        ...
        return pnts, self.vor_polys, self.poly_vertice, self.pos_vertice

#################### ボロノイ図の描写のための関数 ########################################

    def plot_figure(self, bnd, gn_pnts, vor_polys, fig, ax, anim_p, anim_r, anim_t, j):
        ...
        ax.set_aspect('equal')
        # 母点の描画
        ax.scatter( anim_p[anim_t[j]][:-3][:, 0], anim_r[anim_t[j]][:-3][:, 1] )
        # ボロノイ領域の描画
        poly_vor = PolyCollection(verts=anim_r[anim_t[j]], edgecolor="black", facecolors="None", linewidth=1.0)
        ax.add_collection(poly_vor)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        plt.title("state" + str(anim_t[j] + 1))
        fig.canvas.draw()
        # fig.savefig("img.png")
        j += 1
        return j
...
############################## メインで実行される関数 ####################################
    ...
    def main(self):
        ...
        ########## animを保存するためのリストを準備する ##########

        j = 0
        anim_t = range(self.exp_time * 10)
        anim_p = []
        anim_r = []
        ...
        while True:
            ...
            # ボロノイ図の計算・描画        
                self.pnts, vor_polys, poly_vertice, pos_vertice = self.get_voronoi(bnd, self.pnts)
                anim_p.append(self.pnts)
                anim_r.append(vor_polys)

            # グラフの描画
                self.plot_figure(bnd, self.pnts, vor_polys, fig, ax, anim_p, anim_r, anim_t, j)
        ...
        anim = animation.FuncAnimation(fig, coverage_control.plot_figure, 
                fargs=(bnd, anim_p, anim_r, vor_polys, fig, ax), frames=400, interval=100)
        plt.show()
        anim.save("./crazyswarm/ros_ws/src/crazyswarm/turtlebot3/", writer="pillow")
        # anim.save("./crazyswarm/ros_ws/src/crazyswarm/turtlebot3/", writer="imagemagick")
