    ...
    def main(self):
        ...
        while True:
            ...
            # turtlebot3における変換
                ...
                self.Yp = t.transform.translation.z

            # turtlebot3のtf変換
                # t = tf.transformations.euler_from_quaternion((self.data.pose.orientation.x, self.data.pose.orientation.z, -self.data.pose.orientation.y, self.data.pose.orientation.w))
                # self.Xp = t.pose.position.x
                # self.Yp = t.pose.position.z

                # turtlebot3などの障害物の頭上にcrazyflieを真上に持ってくると降下してしまう
                # Y方向に20cmほどずらす作業を行っている
                self.pos_des = list((float(self.Xp), float(self.Yp)+0.2, float(1))) 
                ...
                # デッキをつけていないので、Zに入力を与えないようにしている
                cf.cmdVelocityWorld(np.array([self.X_dot, self.Y_dot, (self.Z_dot-0.1)]), yawRate=self.yaw_dot)
...
"""
〜注意点〜
~/crazyswarm/launch/hover_swarm.launchで設定を変える必要がある
...
# $ export PYTHONPATH=$PYTHONPATH:~/crazyswarm/ros_ws/src/crazyswarm/scripts (PATHを通す)
