...
class coverage_control(Frames_setup, Coverage_controller, Coverage_voronoi):
    ...
    # 着陸させる順番
        self.landing_order=("cf13","cf6","cf5","cf3","cf2","cf1")

        print(self.landing_order)

        self.l=("cf1","cf2","cf3","cf5","cf6","cf13")
                ...
                if num == 5:
                    ...
                    if time.time() - start_time > 90:
                        ...
                        if child_frame in self.fly_cfs:
                            ...
                            elif child_frame == self.landing_order[num]:
...
##################################################################################################
        ...
        # data = {"T": self.T, "X_1": [r[0] for r in self.position_X], "Y_1":[r[0] for r in self.position_Y],
        #         "X_2":[r[1] for r in self.position_X], "Y_2":[r[1] for r in self.position_Y],
        #         "X_3":[r[2] for r in self.position_X], "Y_3":[r[2] for r in self.position_Y],
        #         "X_4":[r[3] for r in self.position_X], "Y_4":[r[3] for r in self.position_Y],
        #         "X_5":[r[4] for r in self.position_X], "Y_5":[r[4] for r in self.position_Y],
        #         "X_6":[r[5] for r in self.position_X], "Y_6":[r[5] for r in self.position_Y],
        #         "X_7":[r[6] for r in self.position_X], "Y_7":[r[6] for r in self.position_Y],
        #         "X_8":[r[7] for r in self.position_X], "Y_8":[r[7] for r in self.position_Y]}
        # df = pd.DataFrame(data)
        # df.to_csv("coverage_ctrl_after_allland_3_{}".format(
        #     datetime.date.today()))
...
#######################################################################################################
        ...
        # plt.show()
        # ani.save("coverage_ctrl_after_allland_3_{}.mp4".format(
        #     datetime.date.today()), writer="ffmpeg")
