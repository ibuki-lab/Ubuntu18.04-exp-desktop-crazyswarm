...
########ボロノイ分割##############################################################################
    ...
    def main(self):
        ...
        self.l=("cf1","cf2","cf3","cf4","cf5","cf6","cf7")
...
###########################################################################################################
        ...
        while True:
            ...
            # 各crazyflieに指令を送る!子フレーム(cf?)と命令対象のcrazyflieが一致している事が絶対条件 !
            for i, cf, child_frame ,crazyflie in zip(range(self.num_cf), self.allcfs.crazyflies, self.child_frames,id):
...
########################################################################################################################################################################
                ...
                if child_frame in self.fly_cfs:
                    ...
                    if voltage > self.cfTypes[self.cfType]["batteryVoltageWarning"]:
                    ...
                    elif voltage < self.cfTypes[self.cfType]["batteryVoltageWarning"]:
...
#####################################################################################################
        ...
        # df.to_csv("coverage_ctrl_battery_{}".format(datetime.date.today()))
...
#######################################################################################################
        ...
        #     fig, self.plot_figure, fargs=(bnd, p1, p2, fig, ax), frames=time, interval=100)
        # plt.show()
        # ani.save("coverage_ctrl_battery.gif", writer="imagemagick")
