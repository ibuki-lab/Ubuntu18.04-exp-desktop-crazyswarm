...
########ボロノイ分割##############################################################################
    ...
    def main(self):
        ...
        while True:
            ...
            if num == 0:
                self.pnts = np.delete(
                    self.pnts, self.child_frames.index('cf5'), 0)
...
###############################################################################################################
            ...
            if time.time() - start_time > 15:
                ...
                if child_frame == self.flying_order[num]:
                ...
                # 元の位置にchild_frameを戻す
                # child=child[:(int(id[2][2])-1)] + [id[2]] + child[(int(id[2][2]))-1:]
                    self.fly_cfs = self.fly_cfs[:(
                        int(id[0][2])-1)] + [id[0]] + self.fly_cfs[(int(id[0][2]))-1:]
