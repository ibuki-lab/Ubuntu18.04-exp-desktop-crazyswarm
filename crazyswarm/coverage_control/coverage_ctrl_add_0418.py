...
########ボロノイ分割##############################################################################
    ...
    def main(self):
        ...
        while True:
            ...
            if num==0:
                self.pnts = np.delete(self.pnts, self.child_frames.index('cf6'), 0)
            ...
            # 着陸中のcrazyflieには命令を送らない or 条件が揃えば離陸命令
                ...
                if num == 0:
                    ...
                    if time.time() - start_time > 10:
                        ...
                        # 元の位置にchild_frameを戻す
                        # self.fly_cfs = self.fly_cfs[:(int(id[0][2])-1)] + [id[0]] + self.fly_cfs[(int(id[0][2]))-1:]
                            self.fly_cfs = self.fly_cfs[:0] + ["cf6"] + self.fly_cfs[0:]
