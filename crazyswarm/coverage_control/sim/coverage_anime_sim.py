...
def main():
    ...
    for t in time:
        ...
        # グラフの描画
        plot_figure(t, bnd, p1, p2, fig, ax)
    ...
    ani = animation.FuncAnimation(
        fig, plot_figure, fargs=(bnd, p1, p2, fig, ax), frames=time, interval=100)
    plt.show()
...
def plot_figure(t, bnd, p1, p2, fig, ax):
    # ボロノイ図の描画
    ax.cla()
    xmin = np.min(bnd[:, 0])
    xmax = np.max(bnd[:, 0])
    ...
    ax.set_aspect('equal')
    # 母点の描画
    ax.scatter(p1[t][:-3][:, 0], p1[t][:-3][:, 1])
    # ボロノイ領域の描画
    poly_vor = PolyCollection(
        verts=p2[t], edgecolor="black", facecolors="None", linewidth=1.0)
    ax.add_collection(poly_vor)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    plt.title("coverage "+str(t+1))
    fig.canvas.draw()
