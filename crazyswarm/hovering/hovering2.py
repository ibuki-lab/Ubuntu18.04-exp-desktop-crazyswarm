...
class const_value_control( Frames_setup):
    ...
    def main(self):
        ...
        # 入力リスト
        input_thrust_list = [0.0, 2.5, 5.0, 7.50, 10.0, 12.50, 15.0, 17.5, 20.0, 22.5, 25.0]
        # input_thrust_list = [0.0, 2.5]
        input_thrust_list = input_thrust_list + sorted(input_thrust_list[:-1], reverse=True)
        for input_thrust in input_thrust_list:
            t = time.time()                
            ...
            while 5 > time.time() - t:
                ...
                self.pwm_register.append(input_thrust*1000)
